...
# 作用：定义 2048 JAX 环境（已修复 observation_mode 参数传递问题）
...
def _slide_and_merge_one_line(line: chex.Array) -> tuple[chex.Array, jnp.float32]:
    """使用 jax.lax.scan 在一个 1D 数组上执行滑动和合并。"""
    ...
    slid_line = jnp.sum(slid_line_parts, axis=0).astype(line.dtype)
    ...
    (merged_line, merged_score, _), _ = lax.scan(
        _merge_step,
        (jnp.zeros_like(line), jnp.float32(0.0), False),
        jnp.arange(line.shape[0])
    )

    # 3. 'slide again'
    _, final_line_parts = lax.scan(_slide_step, 0, merged_line)
    final_line = jnp.sum(final_line_parts, axis=0).astype(line.dtype)
...
class _SingleStream2048:
    ...
    def _add_random_tile(self, rng: chex.PRNGKey, board: chex.Array) -> chex.Array:
        ...
        def _add_tile():
            ...
            choice_idx = jax.random.choice(
                rng_idx,
                jnp.arange(self._rows * self._columns),
                p=empty_indices / num_empty
            )
            new_val = lax.cond(
                jax.random.uniform(rng_val) < 0.9,
                lambda: jnp.uint8(1),
                lambda: jnp.uint8(2)
            )
    ...
    @partial(jax.jit, static_argnums=(0,))
    def get_valid_moves_mask(self, board: chex.Array) -> chex.Array:
        """计算 4 个方向的合法动作掩码"""
        # 上
        new_board_up, _ = self._vmap_slide_merge(board.T)
        is_valid_up = jnp.any(new_board_up.T != board)

        # 下
        new_board_down, _ = self._vmap_slide_merge(jnp.flip(board.T, axis=1))
        is_valid_down = jnp.any(jnp.flip(new_board_down, axis=1).T != board)

        # 左
        new_board_left, _ = self._vmap_slide_merge(board)
        is_valid_left = jnp.any(new_board_left != board)

        # 右
        new_board_right, _ = self._vmap_slide_merge(jnp.flip(board, axis=1))
...
class BatchedJittable2048Environment(batched_jittable_env.BatchedJittableEnvironment):
    """
    2048 环境的批处理包装器
    支持 auto_reset 和 observation_mode 配置
    """
    ...
    def __init__(self, batch_size: int, env_settings: config_dict.ConfigDict):
        ...
        # 只传递游戏逻辑需要的参数给 _SingleStream2048
        game_settings = {
            k: v for k, v in env_settings.to_dict().items()
            if k in ['rows', 'columns']
        }
        self._env = _SingleStream2048(**game_settings)
    ...
    def step(
            self,
            state: batched_jittable_env.EnvState,
            actions: chex.Array,
    ...
    def _single_env_step(
            self,
            env_state: batched_jittable_env.EnvState,
            action: chex.Array,
            auto_reset: bool
    ) -> tuple[batched_jittable_env.EnvState, types.EnvironmentTimestep]:
        ...
        return (
            batched_jittable_env.EnvState(state=final_state, rng=new_rng),
            batched_jittable_env._to_env_timestep(
                self._render(new_state_and_reward),
                new_reward,
                is_terminal
            )
        )
    ...
    def _single_env_reset(
        ...
        return (
            batched_jittable_env.EnvState(state=state_and_reward, rng=new_rng),
            batched_jittable_env._to_env_timestep(
                self._render(state_and_reward),
